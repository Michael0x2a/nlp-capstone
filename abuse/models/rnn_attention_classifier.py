...
class RnnAttentionClassifier(Model[str]):
    base_log_dir = "runs/rnn_attention/run{}"
    ...
    def _make_bidirectional_rnn(self, word_vectors: Any) -> Any:
        with tf.name_scope('bidirectional_rnn'):
            ...
            output_bias = tf.Variable(
                    ...
                    dtype=tf.float32,
                    name='output_bias')
            attention_weight = tf.Variable(
                    tf.random_normal([self.n_hidden_layers * 2], dtype=tf.float32),
                    dtype=tf.float32,
                    name='attention_weight')
            attention_bias = tf.Variable(
                    tf.random_normal([1], dtype=tf.float32),
                    dtype=tf.float32,
                    name='attention_bias')
            ...
            # attention = tf.nn.softmax(tf.stack(
            #         tf.matmul(cell, attention_weight) + attention_bias for cell in outputs))
            cells = tf.stack(outputs, axis=1)
            attention_logits = tf.tanh(
                    tf.reduce_sum(cells * attention_weight, axis=2, keep_dims=True) + attention_bias)
            ...
            # weighted_cells = [cell * attention for cell, attention in zip(cells, attention)]
            weighted_cells = cells * attention
            print(weighted_cells.get_shape())
            avg = tf.reduce_mean(weighted_cells, axis=1)
            print(avg.get_shape())

            # self.cell_predictions = tf.stack(cells, axis=1)
            # cells = tf.stack(outputs, axis=1)
            # self.cell_predictions = tf.squeeze(
            #         tf.matmul(cells, output_weight) + output_bias,
            #         axis=2)
            prediction = tf.matmul(avg, output_weight) + output_bias
            return prediction
